...
const AS_OF_DATE           = 'June 15, 2026';
const DAILY_LABEL          = 'Jun 15';
const WEEKLY_LABEL         = 'WB Jun 15';

const DAYS_WORKED          = 11;
const DAYS_REMAINING       = 11;
const DAYS_TOTAL           = 22;
const DAYS_WORKED_RANGE    = 'Jun 1–5, 8–12, 15';
const DAYS_REMAINING_RANGE = 'Jun 16–19, 22–26, 29–30';
...
const TEAM_PROFILES = {
  'Enterprise': {
    ...
    periods: {
      daily:   { act:{pg1:33,   vip:20,   clubs:32},   tgt:{pg1:150,  vip:150,  clubs:100}  },
      weekly:  { act:{pg1:33,   vip:20,   clubs:32},   tgt:{pg1:600,  vip:600,  clubs:400}  },
      monthly: { act:{pg1:1157, vip:778,  clubs:785},  tgt:{pg1:2400, vip:2400, clubs:1600} },
    },
    conv: { sold:1507, calls:7507 },
    hasTrend: true,
    note: null,
  },
  'Team Anne': {
    ...
    periods: {
      daily:   { act:{pg1:14,  vip:12,  clubs:20},   tgt:{pg1:52,  vip:52,  clubs:35}  },
      weekly:  { act:{pg1:137, vip:114, clubs:123},  tgt:{pg1:206, vip:206, clubs:137} },
      monthly: { act:{pg1:353, vip:308, clubs:238},  tgt:{pg1:822, vip:822, clubs:548} },
    },
    hasTrend: true,
    note: null,
  },
  'Team Jen': {
    reps: 8, color: '#DB2C00',
    periods: {
      daily:   { act:{pg1:11,  vip:4,   clubs:13},  tgt:{pg1:32,  vip:32,  clubs:22}  },
      weekly:  { act:{pg1:74,  vip:64,  clubs:49},  tgt:{pg1:127, vip:127, clubs:85}  },
      monthly: { act:{pg1:180, vip:154, clubs:100}, tgt:{pg1:506, vip:506, clubs:337} },
    },
    hasTrend: false,
    note: null,
  },
  ...
  'Team Mark': {
    reps: 9, color: '#7B726C',
    periods: {
      daily:   { act:{pg1:1,   vip:2,   clubs:0},   tgt:{pg1:32,  vip:32,  clubs:22}  },
      weekly:  { act:{pg1:1,   vip:2,   clubs:0},   tgt:{pg1:127, vip:127, clubs:85}  },
      monthly: { act:{pg1:153, vip:153, clubs:48},  tgt:{pg1:506, vip:506, clubs:337} },
    },
    hasTrend: false,
    note: null,
  },
  'Team Philip': {
    ...
    periods: {
      daily:   { act:{pg1:26,  vip:0,  clubs:22},   tgt:{pg1:9,   vip:9,   clubs:9}   },
      weekly:  { act:{pg1:143, vip:4,  clubs:103},  tgt:{pg1:45,  vip:45,  clubs:45}  },
      monthly: { act:{pg1:286, vip:5,  clubs:222},  tgt:{pg1:200, vip:200, clubs:200} },
    },
    hasTrend: false,
  ...
  'Team Remen': {
    reps: 9, color: '#ECE9E4',
    periods: {
      daily:   { act:{pg1:9,   vip:9,   clubs:4},   tgt:{pg1:36,  vip:36,  clubs:24}  },
      weekly:  { act:{pg1:91,  vip:89,  clubs:22},  tgt:{pg1:143, vip:143, clubs:95}  },
      monthly: { act:{pg1:153, vip:153, clubs:48},  tgt:{pg1:569, vip:569, clubs:379} },
    },
    hasTrend: false,
    note: null,
  },
...
const MONTHLY = {
  ...
  pg1:           [2938, 1842, 2615, 2069, 1707, 1157],
  vip:           [3055, 1859, 2338, 1966,  710,  778],
  clubs:         [1399, 1539, 2120, 2037, 1187,  785],
};

const WEEKLY = {
  labels: ['4/13','4/20','4/27','5/4','5/11','5/18','5/25','6/1','6/8','6/15'],
  pg1:    [459, 559, 327, 298, 401, 415, 556, 634, 490, 33],
  vip:    [518, 420, 195,   0,   9, 296, 405, 440, 318, 20],
  clubs:  [502, 406, 377, 280, 345, 241, 239, 367, 386, 32],
};

const DAILY = {
  labels: ['5/9','5/10','5/11','5/12','5/13','5/14','5/15','5/18','5/19','5/20','5/21','5/22','5/25','5/26','5/27','5/28','5/29','5/30','5/31','6/1','6/2','6/3','6/4','6/5','6/8','6/9','6/10','6/11','6/12','6/13','6/14','6/15'],
  pg1:    [14,4,85,70,86,80,70,101,83,89,77,65,100,102,116,103,91,29,15,139,124,137,110,82,157,97,51,81,70,19,15,33],
  vip:    [0,0,0,0,0,8,1,58,73,73,55,37,84,80,75,76,65,17,8,102,86,92,76,60,119,68,26,55,32,11,7,20],
  clubs:  [14,13,61,73,68,62,63,45,57,54,42,43,36,43,51,40,48,12,9,62,61,72,61,64,63,69,63,81,74,20,16,32],
};

// Team Anne full trend (monthly Feb–present, weekly 4/13–present)
const ANNE_MONTHLY = {
  labels: ['Feb','Mar','Apr','May','Jun MTD'],
  pg1:    [546,  1113, 944, 609, 353],
  vip:    [408,   910, 721, 289, 308],
  clubs:  [240,   305, 287, 353, 238],
};

const ANNE_WEEKLY = {
  labels: ['4/13','4/20','4/27','5/4','5/11','5/18','5/25','6/1','6/8','6/15'],
  pg1:    [225, 244, 137,  94, 131, 156, 213, 211, 137, 5],
  vip:    [184, 167,  80,   0,   1, 110, 178, 189, 114, 4],
  clubs:  [ 84,  70,  74,  98,  74,  72,  77, 109, 123, 7],
};
...
const PHONE_SETTERS_MONTHLY = {
  labels: ['Feb','Mar','Apr','May','Jun MTD'],
  sets:   [689,  1078, 469, 506, 247],
  show:   [336,  457,  288, 304, 131],
...
const PHONE_SETTERS_WEEKLY = {
  ...
  sets:   [101, 71, 141, 120, 119, 106, 121, 126, 0],
  show:   [56,  36,  70,  81,  79,  62,  53,  78, 0],
  closed: [26,  14,  17,  19,  23,  28,  21,  33, 0],
  cash:   [null,null,null,null,null,null,130663,null,null],
};
const PHONE_SETTERS_DAILY = {
  labels: ['6/3','6/4','6/5','6/6','6/7','6/8','6/9','6/10','6/11','6/12','6/13','6/14','6/15'],
  sets:   [33, 33, 14,  4,  2, 21, 37, 22, 19, 23,  1,  3,  0],
  show:   [ 6,  9,  9,  9,  0, 14, 11, 12, 12, 21,  8,  0,  0],
  closed: [ 1,  3,  5,  5,  0,  4,  7,  3,  5,  7,  7,  0,  0],
  cash:   [null,null,null,null,null,null,null,null,24450,null,null,null,null],
};
...
const CUSTOMER_CARE_DAILY = {
  labels:   ['6/3','6/4','6/5','6/6','6/7','6/8','6/9','6/10','6/11','6/12','6/13','6/14','6/15'],
  pg1:      [null,null,null,null,null,null,null,null,null,null,null,null,null],
  vip:      [null,null,null,null,null,null,null,null,null,null,null,null,null],
  clubs:    [null,null,null,null,null,null,null,null,null,null,null,null,null],
  csat_num: [null,null,null,null,null,null,null,null,null,null,null,null,null],
  csat_den: [null,null,null,null,null,null,null,null,null,null,null,null,null],
};
