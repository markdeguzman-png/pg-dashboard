...
const DATA_FILE_TIMESTAMP  = '2026-06-24T16:47:46Z';
const AS_OF_DATE           = 'June 24, 2026';
const DAILY_LABEL          = 'Jun 24';
const WEEKLY_LABEL         = 'WB Jun 22';

const DAYS_WORKED          = 18;
const DAYS_REMAINING       = 4;
const DAYS_TOTAL           = 22;
const DAYS_WORKED_RANGE    = 'Jun 1–5, 8–12, 15–19, 22–24';
const DAYS_REMAINING_RANGE = 'Jun 25–26, 29–30';
...
const TEAM_PROFILES = {
  'Enterprise': {
    ...
    periods: {
      daily:   { act:{pg1:33,   vip:19,   clubs:28},   tgt:{pg1:150,  vip:150,  clubs:100}  },
      weekly:  { act:{pg1:174,  vip:67,   clubs:183},  tgt:{pg1:600,  vip:600,  clubs:400}  },
      monthly: { act:{pg1:1640, vip:1067, clubs:1331}, tgt:{pg1:2400, vip:2400, clubs:1600} },
    },
  ...
  'Team Anne': {
    ...
    periods: {
      ...
      monthly: { act:{pg1:461, vip:368, clubs:410},  tgt:{pg1:822, vip:822, clubs:548} },
    },
    hasTrend: true,
    note: null,
  },
  'Team Jen': {
    ...
    periods: {
      ...
      monthly: { act:{pg1:263, vip:198, clubs:285}, tgt:{pg1:506, vip:506, clubs:337} },
    },
    hasTrend: false,
    note: null,
  },
  ...
  'Team Philip': {
    ...
    periods: {
      ...
      monthly: { act:{pg1:397, vip:8,  clubs:320},  tgt:{pg1:200, vip:200, clubs:200} },
    },
    hasTrend: false,
  ...
  'Team Remen': {
    ...
    periods: {
      ...
      weekly:  { act:{pg1:89,  vip:89,  clubs:21},  tgt:{pg1:143, vip:143, clubs:95}  },
      monthly: { act:{pg1:199, vip:199, clubs:95},  tgt:{pg1:569, vip:569, clubs:379} },
    },
    hasTrend: false,
    note: null,
  },
...
const MONTHLY = {
  ...
  pg1:           [2938, 1842, 2615, 2069, 1707, 1640],
  vip:           [3055, 1859, 2338, 1966,  710, 1067],
  clubs:         [1399, 1539, 2120, 2037, 1187, 1331],
};

const WEEKLY = {
  labels: ['4/13','4/20','4/27','5/4','5/11','5/18','5/25','6/1','6/8','6/15','6/22'],
  pg1:    [459, 559, 327, 298, 401, 415, 556, 622, 463, 381, 174],
  vip:    [518, 420, 195,   0,   9, 296, 405, 428, 309, 234,  67],
  clubs:  [502, 406, 377, 280, 345, 241, 239, 367, 382, 412, 183],
};

const DAILY = {
  labels: ['5/9','5/10','5/11','5/12','5/13','5/14','5/15','5/18','5/19','5/20','5/21','5/22','5/25','5/26','5/27','5/28','5/29','5/30','5/31','6/1','6/2','6/3','6/4','6/5','6/8','6/9','6/10','6/11','6/12','6/13','6/14','6/15','6/16','6/17','6/18','6/19','6/22','6/23','6/24'],
  pg1:    [14,4,85,70,86,80,70,101,83,89,77,65,100,102,116,103,91,29,15,139,124,137,110,82,157,97,51,81,70,19,15,86,67,81,60,70,73,68,33],
  vip:    [0,0,0,0,0,8,1,58,73,73,55,37,84,80,75,76,65,17,8,102,86,92,76,60,119,68,26,55,32,11,7,58,42,48,36,41,36,12,19],
  clubs:  [14,13,61,73,68,62,63,45,57,54,42,43,36,43,51,40,48,12,9,62,61,72,61,64,63,69,63,81,74,20,15,74,68,77,75,72,85,70,28],
};

// Team Anne full trend (monthly Feb–present, weekly 4/13–present)
const ANNE_MONTHLY = {
  labels: ['Feb','Mar','Apr','May','Jun MTD'],
  pg1:    [546,  1113, 944, 609, 461],
  vip:    [408,   910, 721, 289, 368],
  clubs:  [240,   305, 287, 353, 410],
};
...
const PHONE_SETTERS_MONTHLY = {
  labels:   ['Feb','Mar','Apr','May','Jun MTD'],
  sets:     [689,  1078, 469, 506, 411],
  show:     [336,  457,  288, 304, 218],
  closed:   [130,  122,  99,  93,  81],
  cash_rev: [null, null, null, null, 473924],
...
const PHONE_SETTERS_WEEKLY = {
  ...
  cash_rev: [178294,89000,112312,155477,134951,167117,130663,179744,138466,25050],
  cash_ref: [0,     -500, -5000, -6000, -4000, -4000, 0,     -6100, -4000, 0],
};
const PHONE_SETTERS_DAILY = {
  labels:   ['6/3','6/4','6/5','6/6','6/7','6/8','6/9','6/10','6/11','6/12','6/13','6/14','6/15','6/16','6/17','6/18','6/19','6/22','6/23','6/24'],
  sets:     [33, 33, 14,  4,  2, 21, 35, 22, 20, 24,  1,  2, 24, 29, 13, 24, 23, 40, 38,  0],
  show:     [ 6,  9,  9,  9,  0, 14, 11, 12, 12, 21,  8,  0, 16,  7, 12, 11, 11, 14, 17,  1],
  closed:   [ 1,  3,  5,  5,  0,  4,  7,  3,  5,  7,  7,  0,  5,  1,  4,  6,  6,  3,  0,  0],
  cash_rev: [3000,12833,36830,24000,0,29600,32200,17300,24450,37094,39100,0,30250,6500,7500,36200,45666,0,0,0],
  cash_ref: [0,   0,   0,   0,   0,-4100,-2000,0,    0,    0,    0,   0,0,    -2000,0,  0, -2000, 0, 0, 0],
};
...
const PHONE_SETTERS_REPS = [
  ['Jhaziel Gonzales',          {daily:{sets:0,show:0,cash_rev:0,cash_ref:0}, weekly:{sets:1,show:3,cash_rev:0,   cash_ref:0},    monthly:{sets:2, show:26,cash_rev:67978, cash_ref:0}}],
  ['Tommy Tecson',              {daily:{sets:0,show:0,cash_rev:0,cash_ref:0}, weekly:{sets:0,show:2,cash_rev:0,   cash_ref:0},    monthly:{sets:0, show:26,cash_rev:50200, cash_ref:0}}],
  ['Phoebe Estel Ymil Collado', {daily:{sets:0,show:0,cash_rev:0,cash_ref:0}, weekly:{sets:0,show:6,cash_rev:1000,cash_ref:0},    monthly:{sets:4, show:25,cash_rev:53616, cash_ref:-5100}}],
  ['Vian Perez',                {daily:{sets:0,show:0,cash_rev:0,cash_ref:0}, weekly:{sets:3,show:1,cash_rev:3250,cash_ref:0},    monthly:{sets:6, show:13,cash_rev:45350, cash_ref:0}}],
  ['Rafael John Abayan',        {daily:{sets:0,show:0,cash_rev:0,cash_ref:0}, weekly:{sets:1,show:4,cash_rev:0,   cash_ref:0},    monthly:{sets:4, show:32,cash_rev:55833, cash_ref:0}}],
  ['Richard Ian Alvarez',       {daily:{sets:0,show:0,cash_rev:0,cash_ref:0}, weekly:{sets:0,show:3,cash_rev:12250,cash_ref:0},   monthly:{sets:0, show:25,cash_rev:75916, cash_ref:-2000}}],
  ['Heinrich Abarquez',         {daily:{sets:0,show:0,cash_rev:0,cash_ref:0}, weekly:{sets:0,show:6,cash_rev:0,   cash_ref:0},    monthly:{sets:4, show:36,cash_rev:45000, cash_ref:-3000}}],
  ['Dan Emmanuel Nicolas',      {daily:{sets:0,show:0,cash_rev:0,cash_ref:0}, weekly:{sets:0,show:4,cash_rev:2000,cash_ref:0},    monthly:{sets:3, show:17,cash_rev:44700, cash_ref:0}}],
  ['Julius Vizcayno',           {daily:{sets:0,show:0,cash_rev:0,cash_ref:0}, weekly:{sets:0,show:3,cash_rev:6550,cash_ref:0},    monthly:{sets:6, show:18,cash_rev:35330, cash_ref:0}}],
